...
class InputFeaturesFinQA:
    ...
    document_number_location: List[List[
        int]]  # number locations of question and text, number_location[0] is the location of numbers for question, the rest are for texts
...
class ProcessorFinQA:
    ...
    def __init__(self, max_texts_training_retrieval, max_texts_evaluating_retrieval, topn_from_retrieval_texts,
                 max_table_size):
...
class DatasetFinQA(Dataset):
    ...
    def __init__(
        ...
        processor = processors[task](data_args.max_texts_training_retrieval, data_args.max_texts_evaluating_retrieval,
                                     data_args.topn_from_retrieval_texts,
        ...
        with FileLock(f'{cached_features_file}.lock'):
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                if mode == Split.dev:
                    ...
                    examples = processor.get_dev_examples(tokenizer, data_dir) + processor.get_test_examples(tokenizer,
                                                                                                             data_dir)
                else:
                    raise NotImplementedError
...
class ProcessorConv:
    ...
    def _create_examples(self, tokenizer, data_dir, data_type) -> List[InputExampleFinQA]:
        ...
        for data_index, data in tqdm(enumerate(raw_datas), total=len(raw_datas), desc=f'Creating {data_type} examples'):
            ...
            question = " ".join(data["annotation"]["cur_dial"])
            answer = data['annotation']['exe_ans']
            program = data["annotation"]['cur_program']
            gold_texts = list(all_retrieval_texts[example_id]['annotation']['gold_ind'].values())
...
def convert_examples_to_features_finqa(
    ...
    for data_index, example in t:
        ...
        tokenized_texts = text_tokenize_with_numbers(texts, text_b=[question] * len(texts), tokenizer=tokenizer,
                                                     max_length=max_length, headers=table_headers,
        ...
        if max_question_length == 0:
            ...
            tokenized_question = text_tokenize_with_numbers([question], tokenizer=tokenizer, max_length=max_length,
                                                            headers=table_headers, extracted_numbers=extracted_numbers)
            question_input_ids = tokenized_question['input_ids'][0]
            question_attention_mask = tokenized_question['attention_mask'][0]
            question_token_type_ids = tokenized_question['token_type_ids'][
                0] if 'token_type_ids' in tokenized_question else None
            question_question_mask = tokenized_question['question_mask'][0]
        else:
            tokenized_question = text_tokenize_with_numbers([question], tokenizer=tokenizer, max_length=max_length,
                                                            headers=table_headers, extracted_numbers=extracted_numbers)
            question_input_ids = tokenized_question['input_ids'][0] + [tokenizer.mask_token_id] * (
                        max_question_length - len(tokenized_question['input_ids'][0]))
        ...
        each_part_numbers_num = list(map(len, [
            table_numbers] + question_numbers + text_numbers))  # numbers in table , question, text 0, text 1, ...
        if mode == Split.train:
            program_ids, argument_source, argument_source_mask = program_2_ids(example.program, question_numbers[0],
                                                                               text_numbers, table_numbers,
        ...
        id_numbers_map[unique_id] = {'example_id': example.example_id,
                                     'numbers': sum([table_numbers] + question_numbers + text_numbers, [])}
    ...
    with open(os.path.join(data_dir, os.environ['RUN_NAME'].split('@')[0], f'{mode.value}.json'), 'w',
              encoding='utf-8') as output:
        logging.info(f'Writing {mode.value} example id - numbers into file ' + os.path.join(data_dir, os.environ[
            'RUN_NAME'].split('@')[0], f'{mode.value}.json'))
...
def program_2_ids(p: str, question_numbers: List[str], text_numbers: List[List[str]], table_numbers: List[str],
                  constant_list: List[str], operator_list, is_multihiertt=False):
...
def table_tokenize_with_numbers(raw_table: List[List[str]], table_tokenizer: PreTrainedTokenizer, query: str):
    ...
    try:
        table_inputs = table_tokenizer(table=pdtable, queries=query, padding='longest', truncation='drop_rows_to_fit',
                                       max_length=512,
                                       return_tensors='np')
    except ValueError:
        print(query)
        print(pdtable)
    result['input_ids'] = table_inputs['input_ids'][
        0].tolist()  # input ids : (1) x max length (1 because only one query)
    result['attention_mask'] = table_inputs['attention_mask'][0].tolist()  # attention mask : (1) x max length
    result['token_type_ids'] = table_inputs['token_type_ids'][
        0].tolist() if Config.table_model_type == 'tapas' else None  # token_type_ids: (1) x max length x 7 (for tapas)
...
def text_tokenize_with_numbers(text: List[str], tokenizer: PreTrainedTokenizer, text_b: List[str] = None,
                               max_length=512, headers: List[str] = None,
    ...
    for text_index, _text in enumerate(text):
        from_front = True
        while True:
            res = text_tokenize_with_numbers_impl(_text, _text_b=text_b[text_index] if text_b is not None else None,
                                                  tokenizer=tokenizer, max_length=max_length, headers=headers,
                                                  extracted_numbers=extracted_numbers)
...
def text_tokenize_with_numbers_impl(_text: str, tokenizer: PreTrainedTokenizer, max_length: int, _text_b: str = None,
    ...
    if _text_b is not None:
        if Config.model_type == 'bert':
            ...
            _text_input_ids += tokenizer.encode(_text_b,
                                                add_special_tokens=False)  # Attention: DO NOT detect numbers in text b
    ...
    return {'input_ids': _text_input_ids,
            ...
            'token_type_ids': _token_type_ids + [1] * (len(_text_input_ids) - len(_token_type_ids)) if len(
                _token_type_ids) != 0 else None,
