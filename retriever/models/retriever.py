...
class Retriever(RobertaPreTrainedModel):
    def __init__(self, config, roberta: RobertaModel, data_args, roberta2: RobertaModel = None):
        ...
        self.linear = torch.nn.Linear(2 * self.hidden_size, self.hidden_size)
        if data_args.use_ge:
            self.redu = torch.nn.Linear(3 * self.hidden_size, self.hidden_size)
        else:
            self.redu = torch.nn.Linear(2 * self.hidden_size, self.hidden_size)
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, query_input_ids, query_attention_mask,
                query_token_type_ids, text_num, labels, document_mask, example_id, ge_input_ids,
                ge_attention_mask, ge_token_type_ids):

        document_representations = self.roberta(input_ids.long(),
                                                attention_mask=attention_mask,
                                                token_type_ids=token_type_ids)['last_hidden_state']
        if self.roberta2 is None:
            query_representations_raw = self.roberta(query_input_ids.long(),
        ...
        document_representations_ex = self.drop(document_representations)
        query_representations_ex = self.drop(query_representations)
        if ge_input_ids is not None:
            ge_representations_raw = self.roberta(ge_input_ids.long(),
                                                  attention_mask=ge_attention_mask,
                                                  token_type_ids=ge_token_type_ids)['last_hidden_state']
            ge_representations = []
            for instance_index in range(len(text_num)):
                ge_representations.append(
                    ge_representations_raw[instance_index].unsqueeze(0).repeat(text_num[instance_index], 1, 1))

            ge_representations = torch.cat(ge_representations, dim=0).view(len(document_representations), -1,
                                                                           self.hidden_size)
            ge_representations_ex = self.drop(ge_representations)

            interact_qe = self.linear(
                torch.cat([ge_representations_ex.mean(1), query_representations_ex.mean(1)], dim=-1))
            interact_de = self.linear(
                torch.cat([ge_representations_ex.mean(1), document_representations_ex.mean(1)], dim=-1))
            document_representations = self.redu(torch.cat(
                [document_representations, interact_qe.unsqueeze(1).repeat(1, document_representations.size(1), 1),
                 interact_de.unsqueeze(1).repeat(1, document_representations.size(1), 1)],
                dim=-1))
            document_representations = document_representations * mask
            print(123)
        else:
            interact_qd = self.linear(
                torch.cat([document_representations_ex.mean(1), query_representations_ex.mean(1)], dim=-1))
            document_representations = self.redu(torch.cat(
                [document_representations, interact_qd.unsqueeze(1).repeat(1, document_representations.size(1), 1)],
                dim=-1))
            document_representations = document_representations * mask
