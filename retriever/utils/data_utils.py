...
class ProcessorFinQA(DataProcessor):
    ...
    def get_train_examples(self, data_dir, tokenizer, use_ge=False):
        """See base class."""
        logger.info("LOOKING AT {} train".format(data_dir))
        return self._create_examples(f'{data_dir}/train.json', "train", tokenizer, use_ge)
    ...
    def _create_examples(self, data_dir, data_type, tokenizer, use_ge=False) -> List[InputExampleFinQA]:
        ...
        for data_index, data in tqdm(enumerate(raw_datas), total=len(raw_datas),
            ...
            if use_ge:
                try:
                    ...
                    tmp = decoder_for_gpt("As a general rule " + question, 512)
                except:
                    tmp = None
        ...
        with FileLock(f'{cached_features_file}.lock'):
            if os.path.exists(cached_features_file) and not overwrite_cache:
                ...
                if mode == Split.dev:
                    ...
                    examples = processor.get_train_examples(data_dir, tokenizer, self.use_ge)
                elif mode == Split.private_test:
                    examples = processor.get_private_test_examples(data_dir, tokenizer)
                elif mode == Split.dev_and_test:
                    examples = processor.get_dev_examples(data_dir, tokenizer) + processor.get_test_examples(data_dir,
                                                                                                             tokenizer)
                else:
...
class ProcessorConv(DataProcessor):
    ...
    def _create_examples(self, data_dir, data_type, tokenizer, use_ge=False) -> List[InputExampleFinQA]:
        ...
        for data_index, data in tqdm(enumerate(raw_datas), total=len(raw_datas),
            ...
            if data_type != 'private_test':
                gold_texts = list(data["annotation"]['gold_ind'].values())
                gold_texts = list(set(gold_texts))
                other_texts = list(filter(lambda x: x not in gold_texts, texts))
                bm25_results = get_bm25(tokenizer, other_texts, question)
                other_texts = [x['text'] for x in bm25_results]
        ...
        with open(f'../ConvFinQADataset/retriever_{data_type}.json', 'w', encoding='utf-8') as f_obj:
            json.dump(fin_datas, f_obj, sort_keys=False, indent=4, ensure_ascii=False)
...
def convert_examples_to_features_finqa(
    ...
    for data_index, example in t:
        ...
        tokenized_question = tokenizer([question], padding=False, max_length=max_length,
                                       truncation=True, return_overflowing_tokens=False, return_length=True)
        if GE is not None:
            tokenized_ge = tokenizer([GE], padding=False, max_length=max_length,
                                     truncation=True, return_overflowing_tokens=False, return_length=True)
        else:
            tokenized_ge = None
        ...
        if GE is not None:
            ge_input_ids = tokenized_ge['input_ids'][0]
            ge_attention_mask = tokenized_ge['attention_mask'][0]
            ge_token_type_ids = tokenized_ge[
                'token_type_ids'][0] if 'token_type_ids' in tokenized_ge else None
        else:
            ge_input_ids = None
            ge_attention_mask = None
            ge_token_type_ids = None
        trun_count += len(list(filter(lambda x: len(x) >= max_length, texts_input_ids)))
        trun_count += int(len(question_input_ids) >= max_length)
        if GE is not None:
            trun_count += int(len(ge_input_ids) >= max_length)
...
processors = {"FinQA": ProcessorFinQA,"ConvFinQA":ProcessorConv}
